    ...
    with open(fname) as f:
        ...
        while (iter < len(fdata)):
            ...
            if (fdata[iter].startswith("#L")):
                ...
                while (iter < len(fdata) and len(fdata[iter]) > 3 and (
                        (not fdata[iter].startswith("#")) or ("injection" in fdata[iter]) or (
                        "communication" in fdata[iter]))):
...
def GetDirBeamCorrection(corrInfo):
    ...
    for i in range(1, len(corrInfo)):
        cData[corrInfo[i][0] - 1]["PicoAm3"] = cData[corrInfo[i][0] - 1]["PicoAm3"] / (
        cData[corrInfo[i][0] - 1]["I0_BD3"])
...
def GetRXREScanData(inData, inInfo, inScans):
    ...
    for iscan in range(len(inScans[:, 0])):
        ...
        if (snum - 1 <= len(inData)):
            ...
            if (tr2 == 0):
                retList.append(np.transpose(np.array(
                    [inData[i]["MonoEngy"][tr1:], 0 * inData[i]["MonoEngy"][tr1:], 0 * inData[i]["MonoEngy"][tr1:],
                     inData[i]["PicoAm3"][tr1:]])))
            else:
                retList.append(np.transpose(np.array(
                    [inData[i]["MonoEngy"][tr1:-tr2], 0 * inData[i]["MonoEngy"][tr1:-tr2],
                     0 * inData[i]["MonoEngy"][tr1:-tr2], inData[i]["PicoAm3"][tr1:-tr2]])))

            retList[len(retList) - 1][:, 1] = inInfo[i]["Theta"]
            retList[len(retList) - 1][:, 2] = 0.001013546247 * retList[len(retList) - 1][:, 0] * math.sin(
                inInfo[i]["Theta"] * math.pi / 180)
...
def GetRXREScanInfo(inData, inInfo, inScans):
    ...
    for iscan in range(len(inScans[:, 0])):
        ...
        if (snum - 1 <= len(inData)):
            ...
            retList.append(
                [i + 1, ConvPolVtoS(inInfo[i]["Polarization"]), "E", inData[i]["MonoEngy"][0], inInfo[i]["Theta"]])
...
def GetRXRAScanData(inData, inInfo, inScans):
    ...
    for iscan in range(len(inScans[:, 0])):
        ...
        if (snum - 1 <= len(inData)):
            ...
            if (tr2 == 0):
                retList.append(np.transpose(np.array(
                    [0 * inData[i]["Theta"][tr1:], inData[i]["Theta"][tr1:], 0 * inData[i]["Theta"][tr1:],
                     inData[i]["PicoAm3"][tr1:]])))
            else:
                retList.append(np.transpose(np.array(
                    [0 * inData[i]["Theta"][tr1:-tr2], inData[i]["Theta"][tr1:-tr2], 0 * inData[i]["Theta"][tr1:-tr2],
                     inData[i]["PicoAm3"][tr1:-tr2]])))

            retList[len(retList) - 1][:, 0] = inInfo[i]["MonoEngy"]
            retList[len(retList) - 1][:, 2] = 0.001013546247 * inInfo[i]["MonoEngy"] * np.sin(
                math.pi / 180 * retList[len(retList) - 1][:, 1])
...
def GetRXRAScanInfo(inData, inInfo, inScans):
    ...
    for iscan in range(len(inScans[:, 0])):
        ...
        if (snum - 1 <= len(inData)):
            i = snum - 1
            # print(i+1)
            print(inData[i]["Theta"])
            retList.append(
                [i + 1, ConvPolVtoS(inInfo[i]["Polarization"]), "A", inInfo[i]["MonoEngy"], inData[i]["Theta"][0]])
    return np.array(retList)
...
def ApplyDirBeam(inScans, inInfo, corrData):
    for i in range(len(inScans)):
        inScans[i][:, 3] = inScans[i][:, 3] / np.interp(inScans[i][:, 0], corrData[ConvPolStoV(inInfo[i][1])][:, 0],
                                                        corrData[ConvPolStoV(inInfo[i][1])][:, 1])
    return inScans
...
def ApplyGeometryCorrection(inScans, geoData):
    ...
    for i in range(len(inScans)):
        ...
        for j in range(len(d)):
            inScans[i][j, 3] = inScans[i][j, 3] / GaussianIntegral(d[j], p[j], -geoData[1] / 2, geoData[
                1] / 2)  # (scipy.special.erf(eval1)+scipy.special.erf(eval2))
                ...
                if (m in inData[i]):
                    for k in range(0, len(inData[i][m])):
                        if (k == 0 and len(inData[i][m]) > 2):
                            if ((inData[i][m][k] / (inData[i][m][k + 1] + 1e-20) > 2 or inData[i][m][k] / (
                                    inData[i][m][k + 1] + 1e-20) < 0.5) and (
                                    inData[i][m][k + 1] / (inData[i][m][k + 2] + 1e-20) > 2 or inData[i][m][k + 1] / (
                                    inData[i][m][k + 2] + 1e-20) < 0.5)):
                            ...
                            if ((inData[i][m][k] / (inData[i][m][k - 1] + 1e-20) > 2 or inData[i][m][k] / (
                                    inData[i][m][k - 1] + 1e-20) < 0.5) and (
                                    inData[i][m][k] / (inData[i][m][k + 1] + 1e-20) > 2 or inData[i][m][k] / (
                                    inData[i][m][k + 1] + 1e-20) < 0.5)):
...
def XASSubBG(inData, monE, mons, scanNums, elow, ehigh):
    for scan in scanNums:
        ...
        for mon in mons:
            ...
            m = np.sum((inData[i][monE][idx] - xmean) * (inData[i][mon][idx] - ymean)) / np.sum(
                (inData[i][monE][idx] - xmean) ** 2)
            b = ymean - m * xmean
...
def XASSubBGCons(inData, mons, scanNums, elow, ehigh):
    ...
    for i in range(len(inData)):
        if (i + 1 in scanNums):
            for m in mons:
                if (m in inData[i]):
                    inData[i][m] = inData[i][m] - np.average(
                        inData[i][m][np.where((inData[i]["MonoEngy"] > elow) & (inData[i]["MonoEngy"] < ehigh))])
...
def MergeOffRes(ener, XAS, element, eVals):  # ,eL1,eH1,eL2,eH2):
    ...
    pars = [1 / (XAS[-1] + 1e-10), XAS[0] / (XAS[-1] + 1e-10),
            0.1 * (XAS[-1] - XAS[0]) / (ener[-1] - ener[0]) / (XAS[-1] + 1e-10)]
    ...
    XAS = np.hstack(
        (np.ndarray.flatten(orSpec[np.where(orSpec[:, 0] < eL1), 2]), XAS[np.where((ener >= eL1) & (ener <= eH2))]))
    XAS = np.hstack((XAS, np.ndarray.flatten(orSpec[np.where(orSpec[:, 0] > eH2), 2])))
    ener = np.hstack(
        (np.ndarray.flatten(orSpec[np.where(orSpec[:, 0] < eL1), 0]), ener[np.where((ener >= eL1) & (ener <= eH2))]))
...
def KKf1Fromf2(e, im, offset):
    ...
    for i in range(1, len(im) - 1):
        ...
        delta = 0.5 * b1 * np.log(np.fabs((newE - xp) * (newE + xp)) / np.fabs((newE - x0) * (newE + x0))) + m1 * (
                    xp - x0) + 0.5 * newE * m1 * np.log((newE + x0) / (newE + xp)) + 0.5 * newE * m1 * np.log(
            np.fabs(newE - xp) / np.fabs(newE - x0))
        delta = delta + 0.5 * b2 * np.log(
            np.fabs((newE - x0) * (newE + x0)) / np.fabs((newE - xm) * (newE + xm))) + m2 * (
                            x0 - xm) + 0.5 * newE * m2 * np.log((newE + xm) / (newE + x0)) + 0.5 * newE * m2 * np.log(
            np.fabs(newE - x0) / np.fabs(newE - xm))
...
def Getf1Offset(element):
    switcher = {"H": 1, "He": 2, "Li": 3, "Be": 4, "B": 5, "C": 6, "N": 7, "O": 8, "F": 9, "Ne": 10, "Na": 11, "Mg": 12,
